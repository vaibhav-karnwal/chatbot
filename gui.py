...
# Menu bar
    ...
    # Options
        ...
        color_theme.add_command(label="Default",command=self.color_theme_dark_blue) 
       ...
       # color_theme.add_command(label='Mkbhd',command=self.MKBHD)
        ...
        help_option.add_command(label="About V-Bot", command=self.msg)
        help_option.add_command(label="Develpoers", command=self.about)

        self.text_frame = Frame(self.master, bd=6, bg="#263b54")
        self.text_frame.pack(expand=True, fill=BOTH)

        # scrollbar for text box
        self.text_box_scrollbar = Scrollbar(self.text_frame, bd=0, bg="#263b54")
        self.text_box_scrollbar.pack(fill=Y, side=RIGHT)

        # contains messages
        self.text_box = Text(self.text_frame, yscrollcommand=self.text_box_scrollbar.set, state=DISABLED,
                             bd=1, padx=6, pady=6, spacing3=8, wrap=WORD, bg="#263b54",foreground="#FFFFFF", font="Verdana 10", relief=GROOVE,
                             width=10, height=1)
        self.text_box.pack(expand=True, fill=BOTH)
        self.text_box_scrollbar.config(command=self.text_box.yview)

        # frame containing user entry field
        self.entry_frame = Frame(self.master, bd=1, bg="#263b54")
        self.entry_frame.pack(side=LEFT, fill=BOTH, expand=True)

        # entry field
        self.entry_field = Entry(self.entry_frame, bd=1, justify=LEFT, bg="#263b54",foreground="#FFFFFF")
        self.entry_field.pack(fill=X, padx=6, pady=6, ipady=3)
        # self.users_message = self.entry_field.get()

        # frame containing send button and emoji button
        self.send_button_frame = Frame(self.master, bd=0, bg="#263b54")
        self.send_button_frame.pack(fill=BOTH)

        # send button
        self.send_button = Button(self.send_button_frame, text="Send", width=4, relief=GROOVE, bg='#263b54',
                                  bd=1, command=lambda: self.send_message_insert(None), activebackground="#FFFFFF",
                                  foreground="#FFFFFF")
        self.send_button.pack(side=LEFT, pady=6,ipady=2,ipadx=6,padx=4)
